...
function topicupdate (req, res) {
    /* Do the post to a Skynet server */
    var url, xml, log_level, log_msg, userid, id, specid, _req;
    ...
    if (id && url && xml) {
        adjustProgramlistingLanguages(xml, function(err, result) {
            ...
            updateTopicInPressGang(userid, url, id, specid, xml, log_level, log_msg, function (result) {
                res.writeHead(result.code, result.text)
...
function updateTopicInPressGang (userid, url, id, specid, xml, log_level, log_msg, cb) {
    var _cb, _log_level, _log_msg, _url;
    console.log("URL: %s  SpecID : %s", url, specid);
    var DEFAULT_LOG_MSG = (specid) ? 'Updated from "' + Library.Books[url][specid].title + '" Spec ID: ' + specid : "Updated from the Death Star";
    
    console.log('Update topic in PressGang');
    ...
    _log_msg = (log_msg && 'function' != typeof log_msg) ? log_msg : null;
    
    _url = url + pressgang_rest_v1_url + 'topic/update/json';
    ...
    */
    ...
    if (_log_msg == null) _log_msg = DEFAULT_LOG_MSG;
    
    rest.postJson(_url, {   id: id,
   ...
   //             console.log(response.statusCode);
                cb ({code: response.statusCode, text: response.responseText}); 
            });
}
