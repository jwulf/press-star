...
exports.generateStreams = generateStreams; // create/add bookNotificationStreams for each book
...
/*  
    The bookNotificationStreams are used to inform listeners of patch events, as well as build and publishing notifications.
...
function generateStreams(cb){
    ...
    if (! exports.bookNotificationStreams) exports.bookNotificationStreams = {};
    for (var url in Books) {
        if (! exports.bookNotificationStreams[url])
            exports.bookNotificationStreams[url] = {};
        for (var id in Books[url]) {
            if (! exports.bookNotificationStreams[url][id]) {
                exports.bookNotificationStreams[url][id] = new Stream();
                exports.bookNotificationStreams[url][id].writable = exports.bookNotificationStreams[url][id].readable = true;
                exports.bookNotificationStreams[url][id].write = patchStreamWrite;
                exports.bookNotificationStreams[url][id].pipe(Library.LibraryNotificationStream);
                console.log('Book notification stream generated for Content Spec: ' + url + ' ' + id);
            }
        }
    }
    if (cb) return cb();
    ...
    if (exports.replayBuffer[url] && exports.replayBuffer[url][id]) {
        ...
        jsdom.env(my.HTMLPrePublish, [process.cwd() + '/public/scripts/jquery-1.9.1.min.js'], function(err, window) {
            ...
            var applyPatch = function (topicID, newHtml, revision, cb) {
                ...
                ephemeral.write(uuid, 'Applying patch', LOG_CONSOLE);
            ...
            async.series(patchset, function (err, result) {
                ephemeral.write(uuid, 'Finished apply dirty patches', LOG_CONSOLE);
...
function notifyListenersOfPatch(url, id, topicid, html, revision) {
    ...
    exports.bookNotificationStreams[url][id].write({
        topicID: topicid,
        html: html,
        revision: revision
    });
