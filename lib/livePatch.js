var jsdom = require('jsdom').jsdom,
    ...
    streamWrite = require('./../routes/build').streamWrite,
...
function scanBook(skynetURL, id, pathURL, pathURLAbsolute, cb) {
    var buildData, endLoc, topicID, count = 0;
    
    streamWrite(skynetURL, id, 'Generating Topic Dependencies for ' + pathURL);
    ...
    jsdom.env(pathURLAbsolute + '/index.html', [process.cwd() + '/public/scripts/jquery-1.9.1.min.js'], function (err, window){
        if (err) 
            return streamWrite(skynetURL, id, 'Error scanning book for Topic Dependencies: ' + err);
        ...
        streamWrite(skynetURL, id, 'Found ' + count + ' topics');
