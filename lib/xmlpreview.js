...
exports.xmlPreview = function (req, res){    
    ...
    try {    
        ...
        // On some systems (libxslt version)? the rendered
        // topic titles have 'A.' prepended on them
        var _title = $($('.title')[0]).text();
        if (_title) {
            if (_title.substr(0,2) === 'A.') {
                $($('.title')[0]).text(_title.substr(2));
            }
        }
