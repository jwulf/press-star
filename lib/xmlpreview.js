...
exports.xmlPreview = function (req, res){
    ...
    try {
        ...
        if (sectionNum) {
        //if (false) {
            preview = xslt.transform(stylesheet, xmldocument, ['show_remarks' , '1', 'body.only', '1', 'tablecolumns.extension', '0', 'start.numbering.at',
                                        "'" + sectionNum + "'"]); 
        } else {
            preview = xslt.transform(stylesheet, xmldocument, ['show_remarks' , '1', 'body.only', '1', 'tablecolumns.extension', '0']);        
        }  
    }
    catch (err) {   
        console.log(err);
