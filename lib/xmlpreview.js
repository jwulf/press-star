...
exports.xmlPreview = function (req, res){    
    ...
    try {    
        ...
        // On the client side jquery.parseHTML cannot handle it
        var splicer = 0;
        var arraylines = preview.split("\n");
        if (arraylines[0].indexOf('<?xml') !== -1) {splicer ++};
        // DOCTYPE is on the same line as the title sometimes
        if (arraylines[1].indexOf('DOCTYPE') !== -1) {
            var doctypeline = arraylines[1].split(">");
            doctypeline.splice(0,1);
            arraylines[1] = doctypeline.join('>');
        }
