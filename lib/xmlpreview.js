...
exports.xmlPreview = function (req, res){    
    ...
    var replaceAll = function(find, replace, str) {
    	...
    	splicer, arraylines, $, condition, matched, conditions = [], error = null, thisCondition,
    ...
    try {    
        ...
        // Additionally, apply any conditions requested by the client
  		...
  		// To build a profile of conditions in this topic to send back to the client
  		// and also to apply any requested conditions to the preview HTML
  		for (var eachElement = 0; eachElement < elements.length; eachElement ++) {
  			// if this element has a condition
			if (elements[eachElement].attribs && elements[eachElement].attribs.condition) {
				thisCondition = elements[eachElement].attribs.condition;
				console.log(thisCondition);
				// add it to the list of detected conditions to pass back to the client
				if (thisCondition && conditions.indexOf(thisCondition) === -1) { conditions.push(thisCondition); }
				

      			var matched = (condition.indexOf(thisCondition) !== -1);
      			// if the client didn't request the condition, blow it away
      			if (!matched) { 
      				// console.log(thisCondition + ' not matched!');
      				$(elements[eachElement]).remove(); 
	      		}
	      	}
  		}
  		xml = $.html();
