...
function publish (url, id, kerbid, kerbpwd, pguserid, cb) {
    assemble (url, id, {operation: 'publish', kerbid: kerbid, kerbpwd: kerbpwd, pguserid: pguserid}, cb);
}
...
var assembly_line = async.queue(function (task, cb){
    var my = Library.Books[task.my.url][task.my.id],
        uuid = my.uuid;
    ...
    if (task.operation == 'build')
        ...
        function finishedBuild (err, results){
            ...
            if (err) {
                my.emit('notify',{msg: 'There was an error rebuilding. Check the <a target="_blank" href="' + my.buildlogURL + '">build log for this book</a>.'});
            } else {
                my.emit('notify', {bookRebuilt: 'Book Rebuilt'});
            }
...
function csprocessorBuild (task, callback) {
	var csprocessorBuildJob,
     	my = Library.Books[task.my.url][task.my.id],
    ...
    if (operation === 'build') _opts = ['build', '--force-bug-links', '--flatten', '--flatten-topics', '--override', 'brand=deathstar'];
    if (operation === 'publish') { 
        _opts = ['build', '--hide-errors', '--flatten', '--flatten-topics'];
    ...
    csprocessorBuildJob = spawn('csprocessor', _opts, {
...
function unzipCSProcessorArtifact (task, callback) {
    var zipjob, 
        ...
        // remove parens in title, escape spaces to underscores
        bookFilename = my.title.split(' ').join('_').replace(/\(/g, '').replace(/\)/g, ''),
        zipfile = bookFilename + '-publican.zip'; 
    
    my.set('builtFilename', escape(bookFilename));
    my.set('publicanDirectory', directory + '/' + bookFilename);
    
    if (!fs.existsSync(directory + '/' + zipfile)) {  // Abort if zip file does not exist
        ephemeral.write(uuid, directory + '/' + zipfile + ' not found.');
        return callback(directory + '/' + zipfile + ' not found.'); } // Call cb() to free this slot in the async queue

    ephemeral.write(uuid, 'Unzipping publican book in ' + directory, LOG_HEADER);

    // Spawn an unzip process
    zipjob = spawn('unzip', ['-o', zipfile], {
        cwd: directory
        ...
        pressgang.getTopic(url, my.revhistory, 
            function retrievedContentSpecRevisionHistoryTopic(err, result){
                if (err) {
                    ...
                    _rev_history_revision = result.revision;
                    ...
                    if (task.operation == 'publish') {
                        ...
                        client.methodCall('getLatestBuilds', [settings.brewDocsTag, {'__starstar': 1, 'package': packageName}], function (error, result){
                            if (error) {
                                ephemeral.write(uuid, 'Error retrieving package details from Brew. I asked ' +
                                    settings.brewhost + ' for ' + packageName + ' in ' + settings.brewDocsTag +
                                    ' and got: ' + error, [LOG_HEADER, LOG_CONSOLE]);
                                return callback(error); // and bail 
                            }
                            if (result.length === 0) {
                                ephemeral.write(uuid, 'Error retrieving package details from Brew. I asked ' +
                                    settings.brewhost + ' for ' + packageName + ' in ' + settings.brewDocsTag +
                                    ' and got a 0 length response: ' + result, [LOG_HEADER, LOG_CONSOLE]);
                            ...
                            fs.unlink(REV_HISTORY_FILE, function (err){
                                ...
                                fs.writeFile(REV_HISTORY_FILE, _rev_history, 'utf8',  function(err) {
                                    if (err) { // error writing custom Revision_History.xml
                                        ...
                                        $ = null;
                                        // And also write it back to the revision history topic
                                        topicdriver.updateTopicInPressGang({
                                                    id: my.revhistory,
                                                    url: url,
                                                    html: null,
                                                    xml: _rev_history,
                                                    log_level: 1,
                                                    log_msg: 'Updated by Press Star Publisher',
                                                    specid: null,
                                                    userid: task.pguserid,
                                                    revision: _rev_history_revision
                                            }, function revHistoryUpdatedInPressGang() {
    ...
    jsdom.env(htmlFile, [process.cwd() + '/public/scripts/jquery-1.9.1.min.js'], function (err, window){
        ...
        $('.RoleCreateBugPara > a').each(function () {
            ...
            if (fixedRevTopics[topicID]) { // This is a Fixed Revision Topic
                ...
                section.attr('data-pg-topic-id', topicID);
                section.attr('data-pg-topic-rev', fixedRevTopics[topicID]);
                target.attr('target', '_blank');
                target.text('Rev: ' + fixedRevTopics[topicID]);
                // Get specific revision of a topic
                // https://bugzilla.redhat.com/show_bug.cgi?id=950832#c1
                editURL = url.substr(0, url.indexOf('TopicIndex')) + 
                    'pressgang-ccms-ui/#SearchResultsAndTopicView;topicViewData;' + 
                    topicID + '=r:'  + fixedRevTopics[topicID] + ';query;topicIds=' + topicID;
                ...
                $(target.parents('.section')[0]).addClass('sectionTopic pg-topic-id-' + topicID).attr('pg-topic-id', topicID);
            } else {
             /*
...
function updateTopicDependenciesMap (task, callback) {
    ...
    if (livePatch.Topics[url]) for (var topic in livePatch.Topics[url])
        if (livePatch.Topics[url][topic][id]) { delete livePatch.Topics[url][topic][id]; }
    ...
    jsdom.env(htmlToScan, [process.cwd() + '/public/scripts/jquery-1.9.1.min.js'], function(err, window) { 
        ...
        */
        function gotTopicRevisionNumber(_target, _id, _revision) {
            ...
            $(_target.parents('.section')[0]).attr('data-pg-topic-rev', _revision);
            $(_target.parents('.section')[0]).attr('data-pg-topic-id', _id);
        ...
        wrench.copyDirRecursive(my.prePublishDir, my.publishDir, 
            function mvCallback (err){
                if (err) { // Probably disk full, or deleted public directory
                    ...
                    my.buildURL = '/builds/' + my.id + '-' + my.builtFilename;
                }
...
function moveAssemblyLogToPublicDirectory (task, callback) {
    ...
    console.log('Moving assembly log %s to %s', my.assemblylog, dest);
    my.remove('buildID');                
    my.remove('buildID');
...
function runRhpkg (task, callback) {
    ...
    publishJob = spawn('rhpkg', ['publican-build', '--lang', 'en-US'], {
        cwd: my.publicanDirectory
    }).on('exit', function(err) {
    ...
    publishJob.stdout.setEncoding('utf8');
    publishJob.stderr.setEncoding('utf8');
    publishJob.stdout.pipe(ephemeral.streams[uuid].stream);
    publishJob.stderr.pipe(ephemeral.streams[uuid].stream);

    publishJob.stdout.on('data', function(data){
...
function assemble (url, id, operation, ultimate_cb) {
    ...
    if (!Books[url] || !Books[url][id])
        return (ultimate_cb && ultimate_cb('Book ' + url + ' ' + id + ' not found'));
