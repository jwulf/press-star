...
function assemble (url, id, operation, ultimate_cb) {
    ...
    var assembly_line = async.queue(function (task, cb){
        ...
        if (_operation == 'build')
            ...
            function finishedBuild (err, results){
                ...
                if (ultimate_cb) ultimate_cb(); // callback to initiator
            });
        ...
        if (_operation == 'publish') {
            ...
            async.series([
                ...
                publicanBuild,
                getKerberosTicket,
                runRhpkg,// We probably need to spawn the publish job separately from the series, because the series 
                    // tears down all spawned processes when it terminates and releases it queue
    ...
    function getContentSpec (callback) {
    ...
    function csprocessorBuild (callback) {
    ...
    function unzipCSProcessorArtifact (callback) {
    ...
    function customizePublicancfg (callback) {
    ...
    function getCustomRevisionHistory (callback) {
        var uuid = my.uuid,
    ...
    function getCustomEntityFile (callback) {
     var uuid = my.uuid,
        ...
        if (my.entityfile) {
            ...
            pressgang.getTopic(url, my.entityfile, 
                function retrievedContentSpecEntityFileTopic(err, result){
                    if (err) {
                        ephemeral.write(uuid, 'There was an error retrieving the Entity File topic: ' + err, LOG_TIMESTAMP);
                        return callback(err);
                    } else {
    ...
    function publicanBuild (callback) {
        var uuid = my.uuid;
        ...
        var publicanJob = spawn('publican', ['build', '--formats', 'html-single', '--langs', 'en-US'], {
            ...
            if (!err) {  // If everything went OK
                ...
                return callback(err); // Release this async slot
            }
        });
    ...
    function writeEditorMetadataFile (callback) {
    ...
    function createFixedRevisionTopicCollection (callback){
        ...
        */
        ...
        ephemeral.write(uuid, 'Found ' + Object.keys(fixedRevTopics).length -1
    ...
    function writeEditorLinks (callback) {
            ...
            $('.RoleCreateBugPara > a').each(function () {
                ...
                */
                ...
                if (topicsChecked == topicsToCheck) {
                    $('.jsdom').remove();    
                    fs.writeFile(htmlFile, window.document.outerHTML, function(err) {
                        return callback(err);
                    });
                }
            });
    ...
    function playbackDirtyBuffer (callback) {
        livePatch.playBuffer(my.url, my.id, callback);
    }
    ...
    function updateTopicDependenciesMap (callback) {
        var topicID, 
            uuid = my.uuid,
            ...
            */
            function gotTopicRevisionNumber(_target, _id, _revision) {
                ...
                if (count == iterations) { // if we have revision data for all topics in the book
                    livePatch.write(); // write the Topic Dependency map to disk
    ...
    function moveBuildToPublicDirectory (callback) {
    ...
    function getKerberosTicket (callback) {
        
        // Run kdestroy to be doubly, doubly sure
        // Then get a Kerberos ticket
        
        kdestroy(function(err) {
            ephemeral.write(uuid, 'Requesting a new Kerberos ticket', LOG_HEADER);
    ...
    function moveAssemblyLogToPublicDirectory (callback) {
        var PUBLIC_BUILDS_DIR = process.cwd() + '/public/builds/',
    ...
    function kdestroy (callback) { 
        ...
        if (uuid) {
            kdestroyer.stdout.pipe(ephemeral.streams[uuid].stream);
            kdestroyer.stderr.pipe(ephemeral.streams[uuid].stream);
        }
    }


   function runRhpkg (callback) {
    ...
    ephemeral.createStream(_operation, url, id, assembly_log, function (err, new_uuid) {
        ...
        return assembly_line.push({my: my});
    });
}
