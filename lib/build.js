...
function buildingFinished(url, id, err) {
    var Books = jsondb.Books,
        my = Books[url][id];   
    var pathURLAbsolute = BUILDS_DIR + id + '-' + my.builtFilename,
        uuid = my.buildID;
    ...
    my.remove('buildID');                
    my.locked = false;
    if (err) my.builderror = true;
    if (err) {
        my.notify('There was an error rebuilding. Check the <a target="_blank" href="' + my.buildStreamURL + '">build log for this book</a>.'); 
    } else {
