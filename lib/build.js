...
function buildBook(url, id) {
    ...
    var directory = Books[url][id].bookdir,
        ...
        livePatch.patchStreams[url][id].write({notification: true, data: {title: 'On Queue', 
            msg: 'In Build Queue...', buildnotification: true, blink: true, linkURL:  '/buildlog.html?buildid=' + uuid }});   
...
var csprocessorQueue = async.queue(function(task, cb) {
   ...
   ephemeral.write(uuid, 'CSProcessor assembly initiated and in progress\r\n', [LOG_HEADER]);
    ...
    // Send a notification to any open books
    livePatch.patchStreams[url][id].write({notification: true, data: {title: 'Rebuilding...', 
            msg: 'Rebuilding...', buildnotification: true, blink:true, linkURL: '/buildlog.html?buildid=' + uuid }}); 
...
function buildingFinished(url, id, err) {
    ...
    if (err) {
        ...
        livePatch.patchStreams[url][id].write({notification: true, data: {title: 'Build Finished with Error', 
            buildnotification: true, msg: 'Rebuild Book', linkURL : 'rebuild'}}); 
